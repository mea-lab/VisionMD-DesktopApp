...
class HandMovementLeftTask(BaseTask):
    """Analysis task for left hand movement assessment.

    Processes video to extract and analyze left hand movement patterns
    ...
    def extract_landmarks(self) -> tuple:
        ...
        while current_frame_idx < self.task_end_frame_idx:
            ...
            if hand_index == -1 or not detection_result.hand_landmarks[hand_index]:
                ...
                index_finger = BaseTask.get_landmark_coords(hand_landmarks[HandMovementLeftTask.LANDMARKS["INDEX_FINGER_TIP"]], enlarged_coords, original_coords)
                middle_finger = BaseTask.get_landmark_coords(hand_landmarks[HandMovementLeftTask.LANDMARKS["MIDDLE_FINGER_TIP"]], enlarged_coords, original_coords)
                ring_finger = BaseTask.get_landmark_coords(hand_landmarks[HandMovementLeftTask.LANDMARKS["RING_FINGER_TIP"]], enlarged_coords, original_coords)
                wrist = BaseTask.get_landmark_coords(hand_landmarks[HandMovementLeftTask.LANDMARKS["WRIST"]], enlarged_coords, original_coords)
        ...
        if missing_percent > 0.1:
            raise Exception((f"Left hand could not be found in more than 10% of the frames. The video quality may be too low or the video may not be a hand movement task."))
