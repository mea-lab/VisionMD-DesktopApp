    ...
    def extract_landmarks(self) -> tuple:
        ...
        while current_frame_idx < self.task_end_frame_idx:
            ...
            if not detection_result.pose_landmarks:
                ...
                # Use the right knee since this is the right task.
                knee_idx = LegAgilityRightTask.LANDMARKS["RIGHT_KNEE"]
                knee = [
                    landmarks[knee_idx].x * (x2 - x1) + x1,
                    landmarks[knee_idx].y * (y2 - y1) + y1
                ]
                # Average the left and right shoulder coordinates.
                left_shoulder = landmarks[LegAgilityRightTask.LANDMARKS["LEFT_SHOULDER"]]
                right_shoulder = landmarks[LegAgilityRightTask.LANDMARKS["RIGHT_SHOULDER"]]
                shoulder_mid = [
                ...
                left_hip = landmarks[LegAgilityRightTask.LANDMARKS["LEFT_HIP"]]
                right_hip = landmarks[LegAgilityRightTask.LANDMARKS["RIGHT_HIP"]]
                hip_mid = [
        ...
        if missing_percent > 0.1:
            raise Exception((f"Right leg could not be found in more than 10% of the frames. The video quality may be too low or the video may not be a leg agility task."))
